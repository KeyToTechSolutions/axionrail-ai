...
export default function App() {
  ...
  return (
    <div style={{ background:"#05080f", minHeight:"100vh", fontFamily:"'IBM Plex Mono','Courier New',monospace", color:"#b8cce8" }}>
      <style>{`
      ...
      <nav style={{ background:"#0a1020", borderBottom:"1px solid #182845", padding:"0 22px" }}>
        <div style={{ maxWidth:1440, margin:"0 auto", display:"flex" }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} className="nav-btn" style={{
              ...
              letterSpacing:1.2, cursor:"pointer", transition:"all .15s",
      ...
      <main style={{ maxWidth:1440, margin:"0 auto", padding:22, animation:"fadeUp .25s ease" }} key={tab}>
        {tab === "fleet"    && <FleetOverview asset={asset} setAsset={setAsset} setTab={setTab} />}
        {tab === "asset"    && <AssetDeepDive asset={asset} setAsset={setAsset} />}
        {tab === "iot"      && <IoTTelemetry  asset={asset} setAsset={setAsset} tick={tick} />}
        {tab === "fmea"     && <FMEAMatrix />}
